...
file_pairs_info = [
    {
        "truth_folder": 'akurasi 100 normal ke arc flash ke off contact',
        "pred_folder": 'normal ke arc flash ke off contact'
    },
    {
        "truth_folder": 'akurasi100 off contact ke arc ke normal',
        "pred_folder": 'mentah fix off contact ke arc ke normal'
    }
]

def save_df_as_png(df, filename, title, footer_text=None):
    """Menyimpan DataFrame pandas sebagai gambar PNG yang rapi."""
    try:
...
def find_transition_delays(truth_df, pred_df):
    """Menganalisis dan menghitung waktu tunda deteksi untuk transisi kondisi."""
    delays = {}
...
for pair in file_pairs_info:
    ...
    print(f"\nMenguji Skenario:\n1. '{truth_folder}'\n2. '{pred_folder}'\n" + "-"*60)
    for i in range(1, num_synthetic_files + 1):
        truth_filename = os.path.join(truth_folder, f"{truth_folder}{i}.csv")
        pred_filename = os.path.join(pred_folder, f"{pred_folder}{i}.csv")
        try:
        ...
        except Exception as e: print(f"Error saat memproses file '{truth_filename}': {e}")
...
if all_true_labels:
    ...
    # --- Tabel Akurasi per Skenario Transisi (Tabel 4.9) ---
    ...
    # --- Laporan Metrik Klasifikasi (Tabel 4.10) ---
    ...
    try:
        with pd.ExcelWriter(output_excel_path, engine='openpyxl') as writer:
            ...
            # Tambahkan ringkasan gabungan ke Excel
